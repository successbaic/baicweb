...
<%
     String sql=" WHERE ADBULICNO = '"+ADBULICNO+"' and INSYEAR="+INSYEAR;			 	
 	 ArrayList al=GGGL_JYDW_FZJG.getList(sql,request.getParameter("start")); //根据许可证编号和检查年度查询出相关的分支机构信息
...
%>
 <tr style="cursor:hand" align='center' id="tr<%=i%>" class="<%=lc%>" onclick="changeItem('<%=i%>','<%=tb.getRECID()%>','<%=color%>')">
	...
	<td align="center"><%=tb.getBRNAME()%></td><!-- //名称 -->			
	<td align="center"><%=tb.getREGORG()%></td><!-- //登记机关 -->
	<td align="center"><%=tb.getESTDATE()%></td><!-- //设立时间 -->
    <td align="center"><%=tb.getPRILNAME()%></td><!-- //负责人 -->
<%if(SIGN.equals("1"))
  {
%>
    <td align="left"> 
...
<script type="text/javascript">
 function  functionInstance(id){
 	  ...
 	  if(id==1){//添加
	    ...
	    framejumpToC('ggxkz_fzjg_action.jsp',cs2,'确定删除该记录吗?');
	  }
  }
