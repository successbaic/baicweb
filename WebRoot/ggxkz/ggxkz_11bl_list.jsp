...
SysUrlForm.setQueryString("ggxkz_11bl_list.jsp",request);
...
%>
<form name="form1" method="get" action="ggxkz_11bl_list.jsp">
...
%>
<OPTION value=""></OPTION>
<OPTION value="0" <%=typeselected%>>尚未补录</OPTION>
...
if(!ANCHETYPE.equals("")){
	if(ANCHETYPE.equals("0")){
		wheSql+=" AND NOT EXISTS (SELECT 1 FROM GGGL_GGJY WHERE PRIPID=GGGL_JYDW_JBXX.ADBULICNO)";
	}else{
		wheSql+=" AND EXISTS (SELECT 1 FROM GGGL_GGJY WHERE PRIPID=GGGL_JYDW_JBXX.ADBULICNO)";
	}	
}
if(!IFFIRST){
	...
	if(bl){
		...
		+INSYEAR+",AD01057,AD01059,AD01058,AD01060,AD01014,to_date(AD01071,'yyyy-mm-dd'),to_date(AD01072,'yyyy-mm-dd'),AD01064,to_date(AD01017,'yyyy-mm-dd'),AD01034 FROM AD0101 WHERE AD01022 not in (SELECT ADBULICNO FROM GGGL_JYDW_JBXX WHERE INSYEAR="+INSYEAR+") and ad01022 is not null");
	}
	IFFIRST=true;
}	
...
%></td>
<td>
<a onclick="list_click('nw_ggxkz_11control.jsp','blFlag=1&SIGN=3&ADBULICNO=<%=jbxx.getADBULICNO()%>&INSYEAR=<%=jbxx.getINSYEAR()%>','');">
<%
if(ANCHETYPE.equals("") || ANCHETYPE.equals("0")){
%>
