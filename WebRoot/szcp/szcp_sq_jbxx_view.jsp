...
String sb = createHeaderUtil.createHeader_view(REG_NO,YEAR,CerNO,"1",SZCP_SQ_JBXX.getENT_NAME());
out.print(sb);
%>
	...
	<form class="form1" name="form1" method="POST" action="szcp_sq_jbxx_action.jsp">
		...
		<tr>
			...
			<td align='center'><input type="text" id="LIAI_EMAIL"
		...
		<CENTER class="noprint">
			...
			<div class="subImgs" style="display:inline" onclick="view()">下一页</div>
		</CENTER>
	...
	function view() {
		...
		var year = '<%=YEAR%>';
		var cerNo = '<%=CerNO%>';
		window.location.href = "szcp_sq_jygl_view.jsp?REG_NO="+regNo+"&EVAL_YEAR="+year+"&CerNO="+cerNo;
	}
	//动态设置a href
	function selected(v){
	...
	function checkError() {
		...
		if(total_Staff_Qty == null || total_Staff_Qty == "") {
			...
			var flag = fucCheckNUM(total_Staff_Qty);
			if(flag== 0) {
				alert("请输入正确的数字格式");
				return
			}
		...
		if(mng_Staff_Qty == null || mng_Staff_Qty == "") {
			...
			var flag = fucCheckNUM(mng_Staff_Qty);
			if(flag== 0) {
				alert("请输入正确的数字格式");
				return
			}
		...
		if(tech_Staff_Qty == null || tech_Staff_Qty == "") {
			...
			var flag = fucCheckNUM(tech_Staff_Qty);
			if(flag== 0) {
				alert("请输入正确的数字格式");
				return
			}
