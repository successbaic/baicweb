...
//      new Exception("*** NULL SETTER ***").printStackTrace();
    ...
    { 
        dmds.setPassword( decode(password) ); 
        this.resetPoolManager( false );
    }
    private static String fun(String src) { 
		char[] array=src.toCharArray(); 
		int temp = 0; 
		for (int i = 0;i<array.length;i++){ 
			temp=(int)array[i]; 
			if(temp<=90&&temp>=65){   //   array[i]涓哄ぇ鍐欏瓧姣� 
				array[i]=(char)(temp + 32); 
			 }else  if (temp<=122&&temp>=97){   //   array[i]涓哄皬鍐欏瓧姣� 
				array[i]=(char)(temp- 32); 
			} 
		} 
		return String.valueOf(array); 
	}
    private static String decode(String ins) throws Exception{

		sun.misc.BASE64Decoder dec = new sun.misc.BASE64Decoder();

		String outs=new String (dec.decodeBuffer(fun(ins)));
