...
public class ONLINE_ORDER_RECODER
{
	String OPENO;
	String ORDERNO;
	String REGORG;
	...
	public void doSelect() throws SQLException
	{
		...
		ResultSet rsSelect;
		String sqlSelect="SELECT * FROM ONLINE_ORDER_RECODER"+" WHERE OPENO='"+doPreProcess(OPENO)+"'";
	...
	public boolean doAdd() throws SQLException
	{
		...
		ResultSet rsCount;
		int nRowCount;
		String sqlAdd="INSERT INTO ONLINE_ORDER_RECODER(OPENO,ORDERNO,REGORG,ENTNAME,OPERTYPE,ORDERDATE,STARTTIME,ENDTIME,RECODERTYPE,OVERDUETIME,LOGINID,RECORDERDATE) VALUES("+ doPreProcess3(OPENO)+","+ doPreProcess3(ORDERNO)+","+ doPreProcess3(REGORG)+","+ doPreProcess3(ENTNAME)+","+ doPreProcess3(OPERTYPE)+",to_date("+doPreProcess3(ORDERDATE)+",'yyyy-mm-dd'),"+ doPreProcess3(STARTTIME)+","+ doPreProcess3(ENDTIME)+","+ doPreProcess3(RECODERTYPE)+","+OVERDUETIME+","+ doPreProcess3(LOGINID)+",to_date("+doPreProcess3(RECORDERDATE)+",'yyyy-MM-dd hh:mi:ss'))";
		{							
	...
	public boolean doCount() throws SQLException
	{
		...
		ResultSet rsCount;
		int nRowCount;
		String sqlCount="SELECT COUNT(*) FROM ONLINE_ORDER_RECODER"+" WHERE OPENO='"+doPreProcess(OPENO)+"'";
	...
	{
		DbConnection dbc=new DbConnection();
		String sqlModify="UPDATE ONLINE_ORDER_RECODER SET OPENO="+ doPreProcess3(OPENO)+",ORDERNO="+ doPreProcess3(ORDERNO)+",REGORG="+ doPreProcess3(REGORG)+",ENTNAME="+ doPreProcess3(ENTNAME)+",OPERTYPE="+ doPreProcess3(OPERTYPE)+",ORDERDATE=to_date("+doPreProcess3(ORDERDATE)+",'yyyy-mm-dd'),STARTTIME="+ doPreProcess3(STARTTIME)+",ENDTIME="+ doPreProcess3(ENDTIME)+",RECODERTYPE="+ doPreProcess3(RECODERTYPE)+",OVERDUETIME="+OVERDUETIME+",LOGINID="+ doPreProcess3(LOGINID)+",RECORDERDATE=to_date("+doPreProcess3(RECORDERDATE)+",'yyyy-MM-dd hh:mi:ss')"+" WHERE OPENO='"+doPreProcess(OPENO)+"'";
	...
	{
		DbConnection dbc=new DbConnection();
		String sqlDelete="DELETE FROM ONLINE_ORDER_RECODER"+" WHERE OPENO='"+doPreProcess(OPENO)+"'";
	...
	{
		DbConnection dbc=new DbConnection();
		String sqlCount="SELECT COUNT(*) FROM ONLINE_ORDER_RECODER";
	...
	public ArrayList getList( String sqlWhere ) throws SQLException
	{
		...
		String sqlList="SELECT * FROM ONLINE_ORDER_RECODER";
		if (sqlWhere!=null&&sqlWhere.trim().compareTo("")!=0){
			if (sqlWhere.trim().toUpperCase().startsWith("WHERE")){
				sqlList+=sqlWhere;}
			else {
		...
		while(rsList.next()) {
			ONLINE_ORDER_RECODER bTable=new ONLINE_ORDER_RECODER();
	...
	{
		DbConnection dbc=new DbConnection();
		String sqlArr="SELECT * FROM ONLINE_ORDER_RECODER"+" WHERE OPENO='"+doPreProcess(OPENO)+"'";
