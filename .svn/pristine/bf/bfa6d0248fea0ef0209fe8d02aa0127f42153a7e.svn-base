...
%>
	<%=createHeaderUtil.createHeader_view(REG_NO,YEAR,CERNO,"8","") %>
				...
				target="_blank">打印本页</a>
					...
					<c:choose>
						...
						<c:when test='<%="6".equals(lczt.getAPPR_STAT_FLAG()) %>'>
								<div class="subImgs"  style="display:inline" onclick="checkError()">修改</div>
						</c:when>
						<c:otherwise>
								<div class="subImgs"  style="display:inline" onclick="view()">返回</div>
						</c:otherwise>
					</c:choose>
			</CENTER>
	</form>
...
<script type="text/javascript">
		function view() {
			window.location.href = "szcp_jdcx.jsp";
		}	
	function Jdcx(){
