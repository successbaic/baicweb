...
public class QYDJ_CWFZR_SQ
{
	...
	public void doSelect() throws SQLException
	{
		...
		ResultSet rsSelect;
		String sqlSelect="SELECT * FROM QYDJ_CWFZR_SQ"+" WHERE OPENO='"+doPreProcess(OPENO)+"'";
	...
	public boolean doAdd() throws SQLException
	{
		...
		ResultSet rsCount;
		int nRowCount;
		String sqlAdd="INSERT INTO QYDJ_CWFZR_SQ(OPENO,PRIPID,NAME,CERTYPE,CERNO,TEL,MOBTEL,EMAIL,S_EXT_TIMESTAMP) VALUES("+ doPreProcess3(OPENO)+","+ doPreProcess3(PRIPID)+","+ doPreProcess3(NAME)+","+ doPreProcess3(CERTYPE)+","+ doPreProcess3(CERNO)+","+ doPreProcess3(TEL)+","+ doPreProcess3(MOBTEL)+","+ doPreProcess3(EMAIL)+","+ doPreProcess3(S_EXT_TIMESTAMP)+")";
		{							
	...
	public boolean doCount() throws SQLException
	{
		...
		ResultSet rsCount;
		int nRowCount;
		String sqlCount="SELECT COUNT(*) FROM QYDJ_CWFZR_SQ"+" WHERE OPENO='"+doPreProcess(OPENO)+"'";
	...
	{
		DbConnection dbc=new DbConnection();
		String sqlModify="UPDATE QYDJ_CWFZR_SQ SET OPENO="+ doPreProcess3(OPENO)+",PRIPID="+ doPreProcess3(PRIPID)+",NAME="+ doPreProcess3(NAME)+",CERTYPE="+ doPreProcess3(CERTYPE)+",CERNO="+ doPreProcess3(CERNO)+",TEL="+ doPreProcess3(TEL)+",MOBTEL="+ doPreProcess3(MOBTEL)+",EMAIL="+ doPreProcess3(EMAIL)+",S_EXT_TIMESTAMP="+ doPreProcess3(S_EXT_TIMESTAMP)+""+" WHERE OPENO='"+doPreProcess(OPENO)+"'";
	...
	{
		DbConnection dbc=new DbConnection();
		String sqlDelete="DELETE FROM QYDJ_CWFZR_SQ"+" WHERE OPENO='"+doPreProcess(OPENO)+"'";
	...
	{
		DbConnection dbc=new DbConnection();
		String sqlCount="SELECT COUNT(*) FROM QYDJ_CWFZR_SQ";
	...
	public ArrayList getList( String sqlWhere ) throws SQLException
	{
		...
		String sqlList="SELECT * FROM QYDJ_CWFZR_SQ";
		if (sqlWhere!=null&&sqlWhere.trim().compareTo("")!=0){
			if (sqlWhere.trim().toUpperCase().startsWith("WHERE")){
				sqlList+=sqlWhere;}
			else {
		...
		while(rsList.next()) {
			QYDJ_CWFZR_SQ bTable=new QYDJ_CWFZR_SQ();
	...
	{
		DbConnection dbc=new DbConnection();
		String sqlArr="SELECT * FROM QYDJ_CWFZR_SQ"+" WHERE OPENO='"+doPreProcess(OPENO)+"'";
