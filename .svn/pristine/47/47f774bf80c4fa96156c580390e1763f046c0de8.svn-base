...
%>
&nbsp;
<a href='#' onclick="framejumpTo('mcgl_fjxx_do.jsp','action=1&OPENO=<%=OPENO%>&LEREPSIGN=2');">
...
%>
			<a href='../upload/<%=f.getFILEID() %>' target="_blank">查看</a> &nbsp;&nbsp;
			<a href='#' onclick="framejumpToC('mcgl_fjxx_delete.jsp','REMARK=delete&OPENO=<%=f.getOPENO()%>&FILEID=<%=f.getFILEID()%>','确定删除该记录吗?');">删除</a>						
<%
	}	
%>
	</td>
