...
%>
<%=createHeaderUtil.createHeader_view(REG_NO,YEAR,CERNO,"2" ,"") %>
	<!-- form表单 -->
		...
		<table border="1" width="100%" cellpadding="0" cellspacing="0">
			...
			<tr>
				...
				<td class="lc2" colspan="3"><input type="text"
					...
					value="<%=SZCP_SQ_JYGL_XS1.getLAST_ASSET_RMB_W()%>" size="30"
					title="年末资产总额" maxlength=6 id="LAST_ASSET_RMB_W"></input>
				</td>
				<td class="lc2" colspan="3">
					<input type="text" name="LAST_ASSET_RMB_W_TIME"
					value="<%=SZCP_SQ_JYGL_XS2.getLAST_ASSET_RMB_W()%>" size="30"
					title="年末资产总额" maxlength=6 id="LAST_ASSET_RMB_W_TIME"></input>
				</td>
			</tr>
			<tr>
				...
				<td class="lc2" colspan="3"><input type="text"
					...
					value="<%=SZCP_SQ_JYGL_XS1.getLAST_SALE_RMB_W() %>" size="30"
					title="年销售总额（万元）" maxlength=6 id="LAST_SALE_RMB_W"></input>
				</td>
				<td class="lc2" colspan="3"><input type="text"
					name="LAST_SALE_RMB_W_TIME"
					value="<%=SZCP_SQ_JYGL_XS2.getLAST_SALE_RMB_W() %>" size="30"
					title="年销售总额（万元）" maxlength=6 id="LAST_SALE_RMB_W_TIME"></input>
				</td>
			</tr>
			<tr >
			...
			<tr>
				...
				<td class="lc2" colspan="2"><input type="text"
					...
					value="<%=SZCP_SQ_JYGL_XS1.getHOME_SALE_RMB_W() %>" size="15"
					title="境内销售（万元）" maxlength=6 id="HOME_SALE_RMB_W" disblank>
				</td>
				<td class="lc2" colspan="3"><input type="text"
					name="HOME_SALE_RMB_W_TIME"
					value="<%=SZCP_SQ_JYGL_XS2.getHOME_SALE_RMB_W() %>" size="30"
					title="境内销售（万元）" maxlength=6 id="HOME_SALE_RMB_W_TIME" disblank>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="lc2" colspan="2"><input type="text"
					name="OUT_SALE_USD_W" value="<%=SZCP_SQ_JYGL_XS1.getOUT_SALE_USD_W()%>"
					size="15" title="外贸出口（万美元）" maxlength=6 id="OUT_SALE_USD_W"
					disblank>
				</td>
				<td class="lc2" colspan="3"><input type="text"
					name="OUT_SALE_USD_W_TIME" value="<%=SZCP_SQ_JYGL_XS2.getOUT_SALE_USD_W()%>"
					size="30" title="外贸出口（万美元）" maxlength=6 id="OUT_SALE_USD_W_TIME"
					disblank>
				</td>
			</tr>
			<tr>
			...
			<tr>
					...
					<CENTER class="noprint">
						...
						<div class="subImgs" style="display:inline" onclick="view()">下一步</div>
					</CENTER>
					</form> 
...
function view() {
	...
	var year = '<%=YEAR%>';
	var cerNo = '<%=CERNO%>';
	window.location.href = "szcp_sq_htgl_view.jsp?REG_NO="+regNo+"&EVAL_YEAR="+year+"&CerNO="+cerNo;
}
...
function genggai(){
	var test = "${SZCP_SQ_JYGL.QUAL_LEVEL_FLAG}"
	alert(test)
	/* document.getElementById('content').innerHTML="更改内搜索容信息"; */
}
	...
	function doSub() {
		...
		if(COVER_PROV_QTY == null || COVER_PROV_QTY == "") {
			...
			var flag = fucCheckNUM(COVER_PROV_QTY);
			if(flag== 0) {
				alert("覆盖省份数请输入正确的数字格式");
				return
			}
		}
		...
		if(COVER_AREA_QTY == null || COVER_AREA_QTY == "") {
			...
			var flag = fucCheckNUM(COVER_AREA_QTY);
			if(flag== 0) {
		...
		if(INV_PATE_QTY == null || INV_PATE_QTY == "") {
			...
			var flag = fucCheckNUM(INV_PATE_QTY);
			if(flag== 0) {
		...
		if(UTI_PATE_QTY == null || UTI_PATE_QTY == "") {
			...
			var flag = fucCheckNUM(UTI_PATE_QTY);
			if(flag== 0) {
		...
		if(DES_PATE_QTY == null || DES_PATE_QTY == "") {
			...
			var flag = fucCheckNUM(DES_PATE_QTY);
			if(flag== 0) {
		...
		if(REG_MARK_QTY == null || REG_MARK_QTY == "") {
			...
			var flag = fucCheckNUM(REG_MARK_QTY);
			if(flag== 0) {
		...
		if(WEL_MARK_QTY == null || WEL_MARK_QTY == "") {
			...
			var flag = fucCheckNUM(WEL_MARK_QTY);
			if(flag== 0) {
		...
		if(FMS_MARK_QTY == null || FMS_MARK_QTY == "") {
			...
			var flag = fucCheckNUM(FMS_MARK_QTY);
			if(flag== 0) {
