...
%>
<%=createHeaderUtil.createHeader(REG_NO,YEAR,CERNO,"6","") %>
<!-- form表单 -->
	...
	<table border="1" width="100%" cellpadding="0" cellspacing="0">
        ...
        <p style="font-color: #000080;font-size: 11pt;font-weight: bold;"><font color=#000080>社会信用</font>
			...
			<tr>
				...
				<td class="lc2">
						&nbsp;&nbsp;&nbsp;&nbsp;<%=REPORT_YEAR1%>年<input type="text" maxlength=19 id="LAST_DONT_RMB_W_LAST" name="LAST_DONT_RMB_W_LAST" value="<%=gyjz_last.doubleToString(gyjz_last.getLAST_DONT_RMB_W())%>">万元
						&nbsp;&nbsp;&nbsp;&nbsp;<%=REPORT_YEAR2%>年<input type="text" maxlength=19 id="LAST_DONT_RMB_W_NEXT" name="LAST_DONT_RMB_W_NEXT" value="<%=gyjz_next.doubleToString(gyjz_next.getLAST_DONT_RMB_W())%>">万元
				</td>
			...
			<a id="printa"
				...
				<div class="subImgs" onclick="checkError()" style="display:inline">
					下一步
				</div>
			</CENTER>		
</form>			
...
<script type="text/javascript">
	...
	function checkError() {
		...
		var LAST_COURT_INVEST_QTY_NEXT = $("#LAST_COURT_INVEST_QTY_NEXT").val();
		if (LAST_COURT_INVEST_QTY_NEXT == null || LAST_COURT_INVEST_QTY_NEXT == "") {
			alert("请输入未执行法院判决、裁定事项 !");
			return;
		} else {
			if (fucCheckInt(LAST_COURT_INVEST_QTY_NEXT) == 0) {
				alert("未执行法院判决、裁定事项, 请输入数字格式!");
				return;
			}
		}
		
		var LAST_COURT_INVEST_QTY_LAST = $("#LAST_COURT_INVEST_QTY_LAST").val();
		if (LAST_COURT_INVEST_QTY_LAST == null || LAST_COURT_INVEST_QTY_LAST == "") {
			alert("请输入未执行法院判决、裁定事项 !");
			return;
		} else {
			if (fucCheckInt(LAST_COURT_INVEST_QTY_LAST) == 0) {
				alert("未执行法院判决、裁定事项, 请输入数字格式!");
				return;
			}
		}
		
		var LAST_ARBIT_INVEST_NEXT = $("#LAST_ARBIT_INVEST_NEXT").val();
		if (LAST_ARBIT_INVEST_NEXT == null || LAST_ARBIT_INVEST_NEXT == "") {
			alert("请输入未执行仲裁机构裁决事项");
			return;
		} else {
			if (fucCheckInt(LAST_ARBIT_INVEST_NEXT) == 0) {
				alert("未执行仲裁机构裁决事项, 请输入数字格式!");
				return;
			}
		}
		
		var LAST_ARBIT_INVEST_LAST = $("#LAST_ARBIT_INVEST_LAST").val();
		if (LAST_ARBIT_INVEST_LAST == null || LAST_ARBIT_INVEST_LAST == "") {
			alert("请输入未执行仲裁机构裁决事项");
			return;
		} else {
			if (fucCheckInt(LAST_ARBIT_INVEST_LAST) == 0) {
				alert("未执行仲裁机构裁决事项, 请输入数字格式!");
				return;
			}
		}
		
		var LAST_DONT_RMB_W_LAST = $("#LAST_DONT_RMB_W_LAST").val();
		if (LAST_DONT_RMB_W_LAST == null || LAST_DONT_RMB_W_LAST == "") {
			alert("请输入捐赠折合金额  !");
			return;
		} else {
			if (fucCheckNUM(LAST_DONT_RMB_W_LAST) == 0) {
				alert("捐赠折合金额, 请输入数字格式!");
				return;
			}else {
				if(LAST_DONT_RMB_W_LAST.indexOf(".") >= 0) {
					var arr = LAST_DONT_RMB_W_LAST.split(".");
					if(arr[1].length > 6) {
						alert("捐赠折合金额 ,最多为6位小数");
						return
					}
				}else {
					if(LAST_DONT_RMB_W_LAST.length > 12) {
						alert("捐赠折合金额, 最多输入12位整数");
						return
					}
				}
			}
		...
		var LAST_DONT_RMB_W_NEXT = $("#LAST_DONT_RMB_W_NEXT").val();
		if (LAST_DONT_RMB_W_NEXT == null || LAST_DONT_RMB_W_NEXT == "") {
			alert("请输入捐赠折合金额  !");
			return;
		} else {
			if (fucCheckNUM(LAST_DONT_RMB_W_NEXT) == 0) {
				alert("捐赠折合金额, 请输入数字格式!");
				return;
			}else {
				if(LAST_DONT_RMB_W_NEXT.indexOf(".") >= 0) {
					var arr = LAST_DONT_RMB_W_NEXT.split(".");
					if(arr[1].length > 6) {
						alert("捐赠折合金额 ,最多为6位小数");
						return
					}
				}else {
					if(LAST_DONT_RMB_W_NEXT.length > 12) {
						alert("捐赠折合金额, 最多输入12位整数");
						return
					}
				}
