...
%>
<%=createHeaderUtil.createHeader_view(REG_NO,YEAR,CERNO,"7","") %>
		      ...
		      <c:if test="<%=list.size() == 0 %>">
		       ...
		       <c:if test="<%=list.size() != 0 %>">
				...
				<tr>
					...
					<td style="word-break : break-all; width: 50" colspan="2">${temp.HONORDESC}</td>
				</tr>
			</c:forEach>
		      </c:if>
    		</table>
...
function view() {
	...
	var year = '<%=YEAR%>';
	var cerNo = '<%=CERNO%>';
	window.location.href = "szcp_sq_xgzs_view.jsp?REG_NO="+regNo+"&EVAL_YEAR="+year+"&CerNO="+cerNo;
}	
	function addSHRY() {
