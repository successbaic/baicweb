...
function funCheckInForm(form)
{
	...
	for (var i=0;i<objarr.length;i++)
	{
		...
		{	
			...
			//不能为空
			disblank=obj.attributes['disblank'];
			...
			//输入类型
			datatype=obj.attributes['datatype'];
			
			if (datatype!=null && datatype.nodeValue!=null && datatype.nodeValue!="")
			{	
				switch (datatype.nodeValue.toUpperCase())				
				{
					case "DATE":
					{
						if (fucCheckDate(obj.value)==0)
						{							
							svid+="\n["+title+"]:数据格式应为日期型!如:2015-01-01";							
						}
						break;
					}
					case "INT":
