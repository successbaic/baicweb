...
public class GenerateTree 
{
	...
	public GenerateTree()
	{
		...
		try 
		{
			System.out.println("Current Path:"+directory.getCanonicalPath());
		}
		catch(Exception exp)
		{
			exp.printStackTrace();
	...
	public String generateIMG( String sSrc, String sOnclick, String sId, boolean blUp )
	{
		if (blUp==false)
		{
	...
	{
		return generateIMG(sSrc, sOnclick, sId, false);
	}

	/** 生成模块图标的HTML代码 **/
	public String generateModuleImage( int level, int isgroup, String sId, int nChildCount, boolean blUp )
	{
		String sImageFile="";
		String sOnclick="";
		...
		if (level==1)	//总根目录
		{
			...
			if (blUp==true)
			{
				return generateIMG( sImageFile, sOnclick, sId + "i", true );
			}
			else
			{
		...
		if (blUp==true)
		{
			return generateIMG( sImageFile, sOnclick, sId + "ii", true );
		}
		else
		{
	...
	{
		return generateModuleImage( level, isgroup, sId, nChildCount, false );
	}

	/** 生成超级链接代码 **/
	public String generateLink( String sText,String sUrl,String sTitle, int isgroup, String sOnclick, boolean bShow, long MODCODE )
	{
		if (isgroup==1)	//模块组
		{
			if (bShow==true)	//显示子节点
			{
				return "<font title=\""+ sTitle +"\" " + sOnclick +">"+sText+"</font>";
			}
			else
				return "<a href=\"module.jsp?MODCODE="+ MODCODE + "\" target=\"main\">" + sText+"</a>";
		}
		else		//模块
		{
			return "<a href=\""+ sUrl + "\" title=\""+ sText+"\" target=main " + sOnclick +">" + sText+"</a>";
		}
	}

	/** 生成一个模块图标之前的线图标 **/
	public String generateLine( int curlevel, int isgroup, String sId, int nChildCount, boolean blUp )
	{
		String sImageFile="";
		...
		for( int i=2; i<curlevel; i++ )	//循环生成前面的线的图片
		{
			...
			if (blUp==true)
			{
				sReturn+=generateIMG( sImageFile, "", "", true );
			}
			else
			{
		...
		if (blUp==true)
		{
			sReturn+=generateIMG( sImageFile, generateOnclick(curlevel, isgroup, sId, nChildCount), sId + "i", true );
		}
		else
		{
	...
	{
		return generateLine( curlevel, isgroup, sId, nChildCount, false );
	}

	/** 生成onclick事件的代码 **/
	public String generateOnclick(int curlevel, int isgroup, String sId, int nChildCount, boolean blUp)
	{
		String sImageFile="";
		String sImageFileOpen="";
		String sOnclick;
		...
		if (isgroup==1)		//是一个模块组
		{
			...
			sOnclick="onclick=\"switchSysBar(this)\"";
		return sOnclick;
	}
	...
	{
		return generateOnclick(curlevel, isgroup, sId, nChildCount, false);
	}

	public String generateHTML( DbConnection dbc, boolean bShow, String sLoginID ) throws SQLException
	{
		...
		if (rsTree.next())
		{
			...
			sReturn=sTableHeader + "<td style=cursor:hand>"+ generateModuleImage( LEVEL, ISGROUP, "id" + MODCODE, CHILDCOUNT ) + generateLink( MODTITLE, "", BZ, ISGROUP, sOnclick,bShow, MODCODE )+ "</td>" + sTableFooter;
		}
		...
		while (rsTree.next())
		{
			...
			sReturn +=sTableHeader + "<td" + sCursor + ">" + generateLine( LEVEL2, ISGROUP2, "id" + MODCODE2, CHILDCOUNT2 ) + 
				...
				generateLink( MODTITLE2, COMMAND2 +  PARAMETER2, BZ2, ISGROUP2, sOnclick, bShow, MODCODE2 ) + "</td>" + sTableFooter;
	...
	public String generateHTMLWithCheckBox( DbConnection dbc, boolean bShow, String sLoginID ) throws SQLException
	{
		...
		if (rsTree.next())
		{
			...
			//写出结果HTML语句
			sOnclick=generateOnclick( LEVEL, ISGROUP, "id" + MODCODE,CHILDCOUNT, true );
			sReturn=sTableHeader + "<td style=cursor:hand>"+ generateModuleImage( LEVEL, ISGROUP, "id" + MODCODE, CHILDCOUNT, true ) + generateLink( MODTITLE, "", BZ, ISGROUP, sOnclick,bShow, MODCODE )+ "</td>" + sTableFooter;
		}
		...
		while (rsTree.next())
		{
			...
			sOnclick=generateOnclick( LEVEL2, ISGROUP2, "id" + MODCODE2,CHILDCOUNT2, true );
			sReturn +=sTableHeader + "<td" + sCursor + ">" + generateLine( LEVEL2, ISGROUP2, "id" + MODCODE2, CHILDCOUNT2, true ) + 
				generateModuleImage( LEVEL2, ISGROUP2, "id" + MODCODE2, CHILDCOUNT2, true ) + 
				generateLink( MODTITLE2, COMMAND2 +  PARAMETER2, BZ2, ISGROUP2, sOnclick, bShow, MODCODE2 ) + "</td>" + sTableFooter;
	...
	public String generateOutLook( DbConnection dbc, boolean bShow, String sLoginID ) throws SQLException
	{
		String sOnclick;		//生成的onclick事件存放的位置
		String sCursor;			//生成的显示鼠标形状的代码存放位置
		...
		if (rsTree.next())
		{
			...
			//写出结果HTML语句
			sOnclick=generateOnclick( LEVEL, ISGROUP, "id" + MODCODE,CHILDCOUNT );
			sReturn=sTableHeader + "<td style=cursor:hand class=\"root\">"+ generateModuleImage( LEVEL, ISGROUP, "id" + MODCODE, CHILDCOUNT ) + generateLink( MODTITLE, "", BZ, ISGROUP, sOnclick,bShow, MODCODE )+ "</td>" + sTableFooter;
		}
		...
		while (rsTree.next())
		{
			...
			if (ISGROUP2==1)
			{
				sCursor=" class=\"tree\" style=\"cursor:hand;\" ";				
			}
			else
			{
				sCursor=" class=\"mod\" ";
